...
func CurrentCapabilities() CapabilityReport {
	return CapabilityReport{SchemaVersion: 2, Capabilities: []Capability{
		...
		},
		{
			...
			BlockedBy:    []string{"exact post-refund net spend requires a source-native refund settlement feed"}, LastVerified: "2026-09-02",
		},
		{
		...
		},
		{
			...
			Interface:   []string{"cli", "mcp"},
			Implemented: []string{"cash and card request status", "bounded existing request history", "period totals and payment-method aggregates", "private new-file download of an already-completed archive"},
			NextWork:    "validate a completed archive download live and research vendor-receipt reads; request creation remains excluded", NextStepKind: CapabilityNextEvidenceRequired,
			BlockedBy: []string{"the current receipt history has no completed archive item to validate", "no verified vendor-receipt read contract has been observed"}, LastVerified: "2026-09-03",
		},
		{
			...
			Interface:   []string{"cli", "mcp"},
			Implemented: []string{"observed receipt payment-method display names", "per-method receipt counts and amounts", "installment status explicitly unavailable rather than inferred"},
			NextWork:    "capture a redacted sales-slip detail shape and adopt installment months only if explicitly observed", NextStepKind: CapabilityNextEvidenceRequired,
			BlockedBy: []string{"no explicit installment-month field has been observed in the current receipt summary or history shapes"}, LastVerified: "2026-09-03",
		},
		{

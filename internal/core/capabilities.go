...
func CurrentCapabilities() CapabilityReport {
	return CapabilityReport{SchemaVersion: 2, Capabilities: []Capability{
		...
		},
		{
			...
			UserValue:   "show receipt-summary payment-method totals and one order's source-native vendor payment method while keeping lump-sum and installment statistics explicitly unavailable until an installment field is observed",
			Interface:   []string{"cli", "mcp"},
			Implemented: []string{"observed receipt payment-method display names", "per-method receipt counts and amounts", "per-order vendor payment type and cancellation components", "installment status explicitly unavailable rather than inferred"},

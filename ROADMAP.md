...
| P1 | Batch receipts | experimental | Enables accounting, reimbursement, and archive workflows. | Cash/card status, history, summaries, hashed-reference vendor-receipt reads, and private completed-archive download are implemented; validate a completed download live without implementing request creation. |
| P1 | Payment method and installments | experimental | Answers which payment methods funded observed receipt totals without confusing registered cards with usage. | Summary totals and per-order vendor payment types are implemented; keep installment status `unavailable` until an explicit installment-month field is observed. |

...
| --- | --- | --- | --- | --- |
| P0 | Native auth session | available | One human login can support later read-only runs, including headless runs where accepted. Passive status and default verification checks never open a visible fallback; a denied passive check is the typed `access_blocked` state instead of an assumed logout. `doctor` reports browser installation, background-session readiness, and SQLite separately. A user may request `auth verify --headed` explicitly. Browser state remains in the dedicated profile instead of a second cookie file. A cross-platform non-blocking lock returns `profile_in_use`, while a private family/major marker permits browser upgrades and rejects family changes or downgrades before profile launch. Native Linux, macOS, and Windows profile contracts gate tagged releases. | Keep login renewal explicit; profile upgrade, downgrade, family-change, and lock behavior remains a release gate. |
| P0 | Current-browser connection | experimental | Chrome 144+ can expose a running profile without an extension after the user explicitly enables remote debugging and approves the attachment. CLI `current-browser status` and MCP `current_browser_status` passively report `not_enabled` or `endpoint_available` without connecting, prompting, creating a tab, or exposing endpoint metadata; their native Linux, macOS, and Windows contracts gate tagged releases. `--current-browser` and `orders_sync_current_browser` then validate the private loopback endpoint, create only their own allowlisted tab, disconnect without closing Chrome, and never copy session state. This is a broad profile debugging permission, not an unattended default. | Validate the official approval flow and repeated order sync on clean Chrome desktop profiles across macOS, Windows, and Linux. |
| P0 | Ordinary-browser protected-data bridge | experimental | This optional compatibility path uses selected-tab `activeTab` consent when dedicated or approved current-browser modes are unsuitable. The CLI and MCP typed sync paths, single-use authenticated rendezvous, Chrome Native Messaging host, minimal-permission MV3 action, isolated page reader, embedded extension bundle, cross-platform per-user install/doctor/ownership-checked uninstall, digest-authorized resumable upgrades, and SQLite path are implemented. A managed macOS install passed doctor and four consecutive one-page live reads without copying cookies; the final read used Chrome's verified managed `extension_path`. Linux CI runs the per-user file contract and Windows CI runs isolated real-HKCU install/doctor/uninstall; neither is yet a clean desktop Chrome test. The six-target release contract builds allowlisted archives, per-archive SBOMs, checksums, and provenance-ready artifacts. | Validate clean Chrome desktop profiles on Linux and Windows, complete Chrome Web Store privacy review and distribution, and add native OS signing. It must not appear in the default first-run path. |
| P0 | Full order history | available | All later analytics depend on complete, non-looping history. Every sync response identifies the actual acquisition adapter and observed structured-document provenance without accepting caller-supplied source labels. | Keep the private order-model endpoint behind its narrow adapter and synthetic contract tests. |
